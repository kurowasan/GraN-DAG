...
for(nCounter in 1:length(nVec))
{
    ...
    for(pCounter in 1:length(pVec))
    {
        ...
        if(numCores > 1)
        {
            respar <- mcmapply(experiment2parralel,MoreArgs=list(p=p,n=n,pCon=pCon,linear=TRUE, pars = pars),1:numExp, mc.cores = numCores)
        } else
        {
            respar <- mapply(experiment2parralel,MoreArgs=list(p=p,n=n,pCon=pCon,linear=TRUE, pars = pars),1:numExp)
        }
        
        save.image(paste("./results/experiment2SparseP",p,"N",n,".RData", sep=""))
...
save.image(paste("./results/experiment2SparseP",paste(pVec,collapse=""),"N",paste(nVec,collapse=""),".RData", sep=""))
cat("\n")
