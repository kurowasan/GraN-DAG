...
parser.add_argument('--hyperparams-search', action='stor_true',
                    help='Do an hyperparameters search')
...
if args.save_folder:
    ...
    encoder_file = os.path.join(args.save_folder, 'encoder.pt')
    decoder_file = os.path.join(args.save_folder, 'decoder.pt')
...
# ================================================
train_loader, valid_loader, test_loader, ground_truth_G = load_data(args, args.folder, args.file_id, args.batch_size)

w_thresholds = [0.03, 0.05, 0.1, 0.3]

#===================================
# load modules
#===================================
# Generate off-diagonal interaction graph
num_nodes = len(ground_truth_G)
...
triu_indices = get_triu_offdiag_indices(num_nodes)
tril_indices = get_tril_offdiag_indices(num_nodes)
...
def train(epoch, best_val_loss, ground_truth_G, lambda_A, c_A, optimizer):
    ...
    print('Epoch: {0:04d} nll_train: {1:.10f} kl_train: {2:.10f} ELBO_loss: {3:.10f} '
            'mse_train: {4:.10f} shd_trian: {5:.10f} time: {6:.4f}s'.format(epoch,
            np.mean(nll_train), np.mean(kl_train), np.mean(kl_train) + np.mean(nll_train),
            np.mean(mse_train), np.mean(shd_trian), time.time() - t))

    if args.save_folder and (np.mean(kl_train) + np.mean(nll_train)) < best_val_loss:
        torch.save(encoder.state_dict(), encoder_file)
        torch.save(decoder.state_dict(), decoder_file)
        with open(os.path.join(args.save_folder, 'DAG{0}.npy'.format(args.file_id)), 'wb') as f:
            np.save(f, graph)
        if args.hyperparams_search:
            mean_val_loss = np.zeros((len(w_thresholds,)))
            for i, w_threshold in enumerate(w_thresholds):
                mean_val_loss[i] = evaluate(ground_truth_G, lambda_A, c_A, w_threshold)
            with open(os.path.join(args.save_folder, 'hyperparams.npz'), 'wb') as f:
                np.savez(f, val_losses=mean_val_loss, epoch=epoch, w_thresholds=w_thresholds,
                    train_loss=np.mean(kl_train) + np.mean(nll_train),
...
def evaluate(ground_truth_G, lambda_A, c_A, w_threshold):
    encoder.eval()
    decoder.eval()
