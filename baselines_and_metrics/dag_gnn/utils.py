...
def load_data(args, folder, file_id, batch_size=100):
    dag_data = load_from_id(folder, file_id)
    G = nx.DiGraph(dag_data.dag)

    feat_train = torch.DoubleTensor(dag_data.train).unsqueeze(-1)
    feat_valid = torch.DoubleTensor(dag_data.valid).unsqueeze(-1)
    feat_test = torch.DoubleTensor(dag_data.valid).unsqueeze(-1)
